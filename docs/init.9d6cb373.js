...
var getNextPoopTime = function getNextPoopTime(clock) {
  return Math.floor(Math.random() * 2) + 4 + clock;
};
...
var gameState = {
  ...
  handleUserAction: function handleUserAction(icon) {
    if (["HATCHING", "CELEBRATING", "FEEDING", "SLEEP"].includes(this.current)) {
      return;
    }
...
var toggleHighlighted = function toggleHighlighted(icon, show) {
  document.querySelector(".".concat(_constants.ICONS[icon], "-icon")).classList.toggle("highlighted", show);
